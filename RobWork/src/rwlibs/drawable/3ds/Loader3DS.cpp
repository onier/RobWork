...
Model3DPtr Loader3DS::loadModel(const std::string& name)
{
    // holds the main chunk header
    ChunkHeader main;

    _path = StringUtil::getDirectoryName(name);

    // Load the file
    _bin3ds = fopen(name.c_str(), "rb");
    ...
    fseek(_bin3ds, 0, SEEK_SET);

    // Load the Main Chunk's header
    fread(&main.id,sizeof(main.id),1,_bin3ds);
    fread(&main.len,sizeof(main.len),1,_bin3ds);

    // Start Processing
    MainChunkProcessor(main.len, ftell(_bin3ds));
...
void Loader3DS::CalculateNormals()
{
...
void Loader3DS::MainChunkProcessor(long length, long findex)
{
    ChunkHeader h;
...
void Loader3DS::EditChunkProcessor(long length, long findex)
{
    ChunkHeader h;
...
void Loader3DS::MaterialChunkProcessor(long length, long findex, int matindex)
{
    ChunkHeader h;
...
void Loader3DS::MaterialNameChunkProcessor(long length, long findex, int matindex)
{
    // move the file pointer to the beginning of the main
    // chunk's data findex + the size of the header
    fseek(bin3ds, findex, SEEK_SET);
...
void Loader3DS::DiffuseColorChunkProcessor(long length, long findex, int matindex)
{
    ChunkHeader h;
...
void Loader3DS::FloatColorChunkProcessor(long length, long findex, int matindex)
{
    float r;
    float g;
    float b;
...
void Loader3DS::IntColorChunkProcessor(long length, long findex, int matindex)
{
    unsigned char r;
    unsigned char g;
    unsigned char b;
...
void Loader3DS::TextureMapChunkProcessor(long length, long findex, int matindex)
{
    ChunkHeader h;
...
void Loader3DS::MapNameChunkProcessor(long length, long findex, int matindex)
{
    char name[80];
...
void Loader3DS::ObjectChunkProcessor(long length, long findex, int objindex)
{
    ChunkHeader h;
...
void Loader3DS::TriangularMeshChunkProcessor(long length, long findex, int objindex)
{
    ChunkHeader h;
...
void Loader3DS::VertexListChunkProcessor(long length, long findex, int objindex)
{
...
void Loader3DS::TexCoordsChunkProcessor(long length, long findex, int objindex)
{
...
void Loader3DS::FacesDescriptionChunkProcessor(long length, long findex, int objindex)
{
...
void Loader3DS::FacesMaterialsListChunkProcessor(
